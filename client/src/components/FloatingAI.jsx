    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-[#004aad] hover:bg-[#003782] text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center group"
        >
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[400px] h-[560px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden transition-all duration-300">
          ...
          <div className="bg-slate-900 text-white p-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-[#003782] rounded-full flex items-center justify-center">
                <Bot size={16} />
              </div>
              <div>
                <div className="font-bold text-sm">DentAssist AI</div>
                <div className="text-[10px] text-[#6b9ae8] flex items-center gap-1">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-6 h-6 bg-[#e6efff] text-[#004aad] rounded-full flex items-center justify-center shrink-0 mt-0.5">
                    <Bot size={12} />
                  </div>
                )}
            ...
            {typing && (
              <div className="flex gap-2">
                <div className="w-6 h-6 bg-[#e6efff] text-[#004aad] rounded-full flex items-center justify-center">
                  <Bot size={12} />
                </div>
                <div className="bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl rounded-bl-sm">
                  <div className="flex gap-1">
                    <div className="w-1.5 h-1.5 bg-[#4a85d6] rounded-full animate-bounce" />
                    <div className="w-1.5 h-1.5 bg-[#4a85d6] rounded-full animate-bounce" style={{ animationDelay: '0.1s' }} />
                    <div className="w-1.5 h-1.5 bg-[#4a85d6] rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="px-3 pb-2 flex flex-wrap gap-1.5 shrink-0">
              {QUICK_CHIPS.map((chip) => (
                <button
                  ...
                  className="text-[11px] px-2.5 py-1 bg-[#e6efff] text-[#003782] rounded-full hover:bg-[#c2d5f7] transition-colors font-medium"
                >
                  {chip}
                </button>
              ))}
          ...
          <div className="border-t border-gray-100 p-3 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-[#004aad] focus:border-transparent"
                disabled={typing}
              />
              <button
                onClick={() => handleSend()}
                disabled={typing || !input.trim()}
                className="bg-[#004aad] text-white px-3 py-2 rounded-lg hover:bg-[#003782] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <Send size={14} />
              </button>
            </div>
